...
import { GET_COMMENTS, GET_MEMBER, GET_PROPERTIES, GET_SERVICES } from '../../apollo/user/query';
import React, { ChangeEvent, useEffect, useState } from 'react';
import { useMutation, useQuery, useReactiveVar } from '@apollo/client';
import { CREATE_COMMENT, LIKE_TARGET_PROPERTY, LIKE_TARGET_SERVICE } from '../../apollo/user/mutation';
import { CommentInput, CommentsInquiry } from '../../libs/types/comment/comment.input';
import { Messages, REACT_APP_API_URL, getImageUrl } from '../../libs/config';
...
const TaskerDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const [likeTargetProperty] = useMutation(LIKE_TARGET_SERVICE);
	...
	const {
		...
		onCompleted: (data: T) => {
			setAgentProperties(data?.getServices?.list);
			setPropertyTotal(data?.getServices?.metaCounter[0]?.total ?? 0);
		},
	});
	...
	const createCommentHandler = async () => {
		try {
			if (!user._id) throw new Error(Messages.error2); // Add semicolon here
			...
			<Stack className={'agent-detail-page'}>
				<Stack className={'container'}>
					<Stack className={'agent-info'}>
						<img
							src={agent?.memberImage ? getImageUrl(agent?.memberImage) : '/img/profile/defaultUser.svg'}
							alt=""
						/>
							...
							{agentProperties.map((property: Service) => {
								return (
									<div className={'wrap-main'} key={property?._id}>
										<ServiceBigCard service={property} likeServiceHandler={likePropertyHandler} key={property?._id} />
									</div>
								);
							})}
						</Stack>
						<Stack className={'pagination'}>
							{propertyTotal ? (
								<>
									...
									<span>
										Total {propertyTotal} servic{propertyTotal > 1 ? 'es' : 'e'} available
									</span>
								</>
							) : (
...
export default withLayoutBasic(TaskerDetail);
