...
const MemberFollowings = (props: MemberFollowingsProps) => {
	...
	const {
		...
		skip: !followInquiry?.search?.followerId, // Skip query if no followerId
		notifyOnNetworkStatusChange: true, // Update UI on network changes
		onCompleted: (data: any) => {
			// Type the data argument
			setMemberFollowings(data?.getMemberFollowings?.list || []); // Handle undefined values
				...
				<Stack className="follows-list-box">
					...
					{memberFollowings.map((follower: Following) => {
						const imagePath: string = follower?.followingData?.memberImage
							? getImageUrl(follower?.followingData?.memberImage)
