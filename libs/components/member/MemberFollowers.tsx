...
const MemberFollowers = (props: MemberFollowsProps) => {
	...
	const {
		...
		skip: !followInquiry?.search?.followingId, // Skip query if no followingId
		notifyOnNetworkStatusChange: true, // Update UI on network changes
		onCompleted: (data: any) => {
			// Type the data argument
			setMemberFollowers(data?.getMemberFollowers?.list || []); // Handle undefined values
	...
	if (device === 'mobile') {
		return <div>HOME CARE EXPERTS FOLLOWS MOBILE</div>;
	} else {
		return (
				...
				<Stack className="follows-list-box">
					...
					{memberFollowers.map((follower: Follower) => {
						const imagePath: string = follower?.followerData?.memberImage
							? getImageUrl(follower?.followerData?.memberImage)
